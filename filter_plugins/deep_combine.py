...
class FilterModule(object):
    ...
    def deep_combine(self, d1, d2):
        ...
        #     if type(d1) is not dict: d1 = { d1 }
        #     if type(d2) is not dict: d2 = { d2 }
        # except Exception as e:
        #     raise errors.AnsibleFilterError(
        #         'deep_combine plugin error (dict conversion fail): {0}, '
        #         'dictA={1}, dictB={2}'.format(str(e), str(d1), str(d2))
        #     )
        ## Instead of attempting to convert to a dict, just ensure that they are dicts
        ...
        try:
            ...
            raise errors.AnsibleFilterError(
                'deep_combine plugin error: {0}, dictA={1},'
                'dictB={2}'.format(str(e), str(d1), str(d2))
            )
    ...
    def _not_dict(self, d):
        raise errors.AnsibleFilterError(
            'deep_combine plugin error - not a dict: {0}'.format(str(d))
        )
